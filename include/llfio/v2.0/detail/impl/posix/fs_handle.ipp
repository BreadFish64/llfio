          ...
          if(h.is_directory())
          {
            ...
            */
            ...
            if(tffd >= 0)
            {
              ...
              char in[64];
              snprintf(in, sizeof(in), "thread-self/fd/%d", tffd);
...
result<void> fs_handle::relink(const path_handle &base, path_view_type path, bool atomic_replace, deadline d) noexcept
{
  ...
  if(h.flags() & handle::flag::anonymous_inode)
  {
    ...
    char _path[PATH_MAX];
    snprintf(_path, PATH_MAX, "thread-self/fd/%d", h.native_handle().fd);
    OUTCOME_TRY(const native_handle_type proc_base, get_proc_base());
...
result<void> fs_handle::link(const path_handle &base, path_view_type path, deadline d) noexcept
{
  ...
  if(ENOENT == errno)
  {
    ...
    char in[64];
    snprintf(in, sizeof(in), "thread-self/fd/%d", h.native_handle().fd);
    OUTCOME_TRY(const native_handle_type proc_base, get_proc_base());
